...
public class PanelInGame extends JPanel {
		...
		for (int i = 0; i < 19; i++) {
			for (int j = 0; j < 22; j++) {
				...
				case 25:
					...
					break;
				case 26:
					g.drawImage(myimage.getCase("case26"), i * 28, j * 28, this);
					break;
				}
			}
		}
	...
	public void drawOptions(Graphics g) {
		...
		TestTimer2 ts = new TestTimer2();
		g.drawString("Questions:", this.getWidth() - 105, 2 * (this.getHeight() / 3) - 28);
		g.drawString(""+TestTimer2.counter, this.getWidth() -105, 2 * (this.getHeight() / 3) - 98);
		g.drawString("Timer:", this.getWidth() -105, 2 * (this.getHeight() / 3) - 128);
		if (bonus[0])
